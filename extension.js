...
function findChunkBounds(document, cursorLine) {
  ...
  for (let i = cursorLine; i < document.lineCount; i++) {
    const text = document.lineAt(i).text;
    if (i > startLine && CHUNK_END.test(text)) {
      endLine = i;
      break;
    }
  }
...
function activate(context) {
    ...
    vscode.commands.registerCommand("rChunk.selectWholeChunk", () => {
      ...
      if (!bounds) {
        vscode.window.showInformationMessage(
          "Cursor is not inside an R chunk.",
        );
        return;
      }
      ...
      const end = new vscode.Position(
        bounds.endLine,
        doc.lineAt(bounds.endLine).text.length,
      );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("r.viewObject", async () => {
      ...
      if (!word) {
        vscode.window.showInformationMessage(
          "No word or selection found at cursor.",
        );
        return;
      }
